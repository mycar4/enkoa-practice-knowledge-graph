...
Python 3.12 환경에서 다음 패키지를 설치하고 해당 환경을 Jupyter 커널로 선택하세요.

```bash
python -m pip install -r requirements.txt
python -m ipykernel install --user --name kg-pipeline --display-name "KG Pipeline"
```
...
`SimpleKGPipeline`은 아직 기능과 사용법이 바뀔 수 있는 실험적 API입니다. `requirements.txt`에 적은 버전으로 실습합니다.
