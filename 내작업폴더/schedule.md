...
| **v0.2 Step 3** | **Streamlit 대시보드 UI 연동 및 팩트 상세 패널 구축** | 프론트엔드 연동 | **🔴 검수 불합격 · 수정 진행 중 (In Revision)** | • `app_dart_trace_dashboard.py` (탭 실행 순서 상태 덮어쓰기 버그 수정, 콜백 기반 독립 선택 연동, 추정값 배제 및 `UNKNOWN` 표기 적용 중) |
