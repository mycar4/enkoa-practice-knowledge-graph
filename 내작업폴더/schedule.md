...
> **최종 갱신 일시**: 2026-09-04 15:18 (KST - 사용자 피드백 정석 순서 전면 동기화)  
> **공식 플랫폼 정체성**: **"오를 종목 맞히기가 아닌, 내 보유 종목의 사실·변화·리스크·공식 근거를 판단하는 투자 의사결정 보조 도구"**  
> **현재 마일스톤**: **v0.4 Step 6 (엔티티 해소 무결성 감사) & v0.5 DRY-RUN 및 실행기 v2.1 안전성 보강 완료 🟢**  
> **차기 최우선 착수**: **v0.6 (단일 기업 대상 4단 의사결정 코어 엔진) ➔ v0.7 (재무/공식 타임라인) ➔ v1.0 (True GraphRAG 완성) ➔ v1.1 (포트폴리오/시세 확장) ⚪**  
...
| **v0.5** | **경제적 보유 관계 생성 DRY-RUN 및 실행기 안전성 보강** | PASS 19건 | **🟢 DRY-RUN 및 보강 완료 (2026-09-04)** | • 정규식/숫자형 3/3 일치 전수 검증 통과 (탈락 0건)<br>• 사명 변경 98건 `DEFERRED_HISTORICAL_NAME` 보류<br>• 실행기 v2.1 4대 안전성(표준 XPath, 단일 트랜잭션, 3분할 no-op 회계, 2023년 공시일 고정) 장착<br>• *사용자 승인 시 Cloud Aura 적재 대기* |
| **v0.6** | **메뉴 2 전면 개편: 4단 의사결정 리포트 코어 엔진** | 단일 기업 대상 | **⚪ 차기 최우선 착수** | • **"사실 / 해석 / 원문 근거 / 다음 확인 항목"** 4단 구조 응답 엔진 배포<br>• A사 입력 시 CB/BW 희석 위험, 유상증자 목적, 대주주 변동 시나리오 분석 |
| **v0.7** | **OpenDART XBRL 재무제표 & 공식 채널 타임라인 결합** | 상장사 전수 | **⚪ 대기** | • OpenDART 재무 API 연계 (부채비율, 유동비율, 영업이익) 사실 계층 바인딩<br>• DART 공시 + KIND 공시 + 기업 공식 IR 타임라인 결합 |
| **v1.0** | **자본이벤트 512차원 Vector Index & Day 35 Hybrid GraphRAG** | 전체 자본이벤트 | **⚪ 기본 서비스 완성** | • 자본조달 목적 text-embedding-3-small 임베딩 및 Neo4j Vector Index 구축<br>• GDS Louvain 커뮤니티 사전 필터 & MinMax 70:30 (유사도:PageRank) 리랭커 가동 |
...
flowchart TD
    ...
    subgraph Phase_v06_v10 ["💎 기본 서비스 완성: 코어 의사결정 엔진 & True GraphRAG (차기 집중)"]
        V06["[v0.6] 메뉴 2 전면 개편: 4단 의사결정 리포트\n- 단일 기업 대상 사실/해석/원문근거/다음확인 생성기"]
        V07["[v0.7] OpenDART XBRL 재무 & 공식 타임라인\n- 부채/유동비율 결합 + DART/KRX/IR 출처별 결속"]
