...
> **기준 일시**: 2026-09-01 10:55 (KST)  
> **현재 마일스톤**: **v0.2 Step 3 (Streamlit 대시보드 UI 및 팩트 상세 패널 구축) 진행 중 / v0.3 명세서 확정**  
...
| **v0.2 Step 3** | **Streamlit 대시보드 UI 연동 및 팩트 상세 패널 구축** | 프론트엔드 연동 | **🔄 진행 중** | • `app_dart_trace_dashboard.py` (우측 팩트 상세 패널, DART 뷰어 URL 연동, 이중 상태 배지 표기) |
| **v0.3** | **`DS005` 기업 주요 이벤트 (CB·BW발행, 합병, 분할, 주식양수도, 소송)** | 100개사 확장 | **🟢 명세 확정** | • `00_DART_Trace_지식그래프_온톨로지_및_데이터구조_명세서_v0.3.md`<br>• `:DART_CapitalEvent`, `:ISSUED_CB`, `:MERGED_WITH`, `:ACQUIRED_STAKE` 승격 |
...
* **신규 노드**: `:DART_CapitalEvent` (CB/BW 발행, 합병, 분할, 주식양수도 이벤트)
* **신규 관계**: `[:ISSUED_CB]`, `[:SUBSCRIBED_CB]`, `[:MERGED_WITH]`, `[:SPUN_OFF_FROM]`, `[:ACQUIRED_STAKE]`, `[:TENDER_OFFER_FOR]`, `[:SUED_BY]`
* **신규 제약조건**:
  ```cypher
