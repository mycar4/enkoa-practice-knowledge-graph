...
| **Step 3**<br>(대시보드 UI) | **Streamlit UI 연동 & 팩트 패널** | • **테이블 인터랙션**: 지분/출자 행 클릭 시 우측 팩트 상세 패널 즉시 갱신<br>• **이중 상태 배지**: `doc_status` + `verification_status` + `is_current` 정상 노출<br>• **날짜 체계**: `as_of_date`(결산일) vs `reported_on`(접수일) 분리 표기<br>• **DART 원문 연동**: `viewer_url` 클릭 시 금감원 공식 뷰어 새 탭 호출 | **🟢 합격 (완료)** |
