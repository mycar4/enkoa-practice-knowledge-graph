...
|:---:|---|---|:---:|
| **Step 1**<br>(공시 인덱스) | **`DS001` 공시 검색 수집** | • **[검수 스냅샷]** `:DART_Disclosure` `21,538건` 전건 적재<br>• **[현재 파일럿 DB]** `:DART_Disclosure` `17,443건`<br>• `rcept_no` 14자리 UNIQUE 제약조건 활성화<br>• `doc_status` (`NORMAL` / `CORRECTED` / `WITHDRAWN`) 문서 상태 분기 | **🟢 합격 (완료)** |
