...
|:---:|---|---|:---:|
| **Step 1**<br>(공시 인덱스) | **`DS001` 공시 검색 수집** | • `:DART_Disclosure` 노드: `21,538개` 전건 적재<br>• `rcept_no` 14자리 UNIQUE 제약조건 활성화<br>• `doc_status` (`NORMAL` / `CORRECTED` / `WITHDRAWN`) 문서 상태 분기 | **🟢 합격 (완료)** |
| **Step 2**<br>(정형 지분망) | **`DS004` (지분) + `DS002` (출자)** | • `:OWNS_STAKE` 총 `723건` (베이스라인 319건 100% 보존)<br>• `:INVESTED_IN` 총 `116건` (장부가액/지분율 >0 검증)<br>• 정확히 1건 매칭(`len==1`) 법인만 VERIFIED 승격<br>• 미식별 개인/미매칭 법인 `3,893건` ➔ `candidate_queue.jsonl` 영구 격리 | **🟢 합격 (완료)** |
