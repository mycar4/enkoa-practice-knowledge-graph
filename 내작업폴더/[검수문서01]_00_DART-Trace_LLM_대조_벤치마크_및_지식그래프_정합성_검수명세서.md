...
| **Step 3**<br>(대시보드 UI) | **Streamlit UI 연동 & 팩트 패널** | • **테이블 인터랙션**: 지분/출자/공시/후보큐 테이블 행 클릭 시 단일 팩트 패널 실시간 연동<br>• **출처 사실성**: 임의 접수번호 완전 제거, 미연결 시 `UNLINKED` 표기 및 DART 링크 숨김<br>• **이중 상태 배지**: 실제 `DART_Disclosure` 조인 결과 기반 (`doc_status` + `verification_status`)<br>• **후보 큐 상세**: `CANDIDATE` 전용 뷰 및 원본 명칭/보류 사유 노출<br>• **5대 케이스 실측 검증**: 출처 유/무 지분, 출자, 공시, 후보 큐 정상 연동 | **🟢 합격 (완료)** |
