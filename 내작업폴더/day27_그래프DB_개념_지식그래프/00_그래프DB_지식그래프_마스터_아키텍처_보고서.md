...
```mermaid
flowchart TD
    ROOT["0. 온톨로지 스키마 설계\n(클래스 정의, 도메인/레인지 제약 헌법)"]
    
    LPG["1. LPG 속성 그래프 엔진\n(Node, Label, Property, Edge)"]
    ADJ["인덱스 프리 인접성\n(Adjacency List 구축)"]
    TRAV["1·2-Hop 고속 순회\n(실시간 추천 & 사기 탐지)"]
    
    TRIPLE["2. RDF 시맨틱 지식 그래프\n(주어 - 술어 - 목적어)"]
    SPARQL["SPARQL 1.1 쿼리 엔진\n(조인, UNION, OPTIONAL, /)"]
    
    EXTRACT["3. GraphRAG 지식 인텔리전스\n(서브그래프 사실 팩트 추출)"]
    LLM["LLM 에이전트\n(환각 0% 무결점 팩트 추론)"]

    ROOT -->|실무 분석 및 추천 구현| LPG
    LPG --> ADJ --> TRAV
    
    ROOT -->|표준 사실 및 지식공유 구현| TRIPLE
    TRIPLE --> SPARQL
...
#### 📐 텍스트 조감도 (어떤 뷰어에서도 100% 즉시 표시)

```text
┌───────────────────────────────────────────────────────────────────────────────────────────────────────────┐
│ 0. 최상위 원천 설계도: 온톨로지 거버넌스 (Ontology Schema Foundation)                                      │
│  [📐 온톨로지 개념 헌법: '인물은 직업을 갖고, 고객은 음악을 청취한다'] (Class, Domain, Range 제약 정의)         │
└────────────────────────────────────────────────┬──────────────────────────────────────────────────────────┘
                                                 │
                   ┌─────────────────────────────┴─────────────────────────────┐
                   │ (구현 모델 A)                                             │ (구현 모델 B)
...
flowchart TD
    ...
    Gate -->|클래스/도메인 통과| Pass["정제된 지식 그래프로 적재"]
    Gate -->|규격 미달| Drop["오염 데이터 폐기 및 차단"]
```

```text
[외부 원천 데이터] ──> [온톨로지 검증 헌법 (Pydantic)] ──┬──(통과)──> [정제된 지식 그래프 적재]
                                                          └──(미달)──> [오염 데이터 폐기 및 차단]
```
...
```mermaid
flowchart LR
    U1["사용자 노드 (u1: 민서)"] -->|들었다| S1["곡 노드 (s1: 밤편지)"]
    S1 -->|부른가수| A1["아티스트 노드 (a1: 아이유)"]
```

```text
[사용자 노드 (u1: 민서)] ──(들었다)──> [곡 노드 (s1: 밤편지)] ──(부른가수)──> [아티스트 노드 (a1: 아이유)]
```
...
```mermaid
flowchart LR
    A["나 (u1: 민서)"] -->|1홉: 들은 곡| B["s3: Ditto"]
    B -->|역방향: 함께 들은 사람| C["u2: 준우"]
    C -->|2홉: 준우가 들은 다른 곡| D["s4: OMG (2홉 추천)"]
```

```text
[나 (u1: 민서)] ──(1홉: 들은 곡)──> [s3: Ditto] ──(역방향: 함께 들은 사람)──> [u2: 준우] ──(2홉)──> [s4: OMG (2홉 추천)]
```
...
```mermaid
flowchart LR
    S["주어 (Subject: ex:민서)"] -->|술어: ex:들었다| O["목적어 (Object: ex:s3)"]
    O -->|술어: ex:부른가수| A["목적어: ex:뉴진스"]
    A -->|술어: ex:국적| L["리터럴 값: 대한민국"]
```

```text
[주어: ex:민서] ──(ex:들었다)──> [목적어: ex:s3] ──(ex:부른가수)──> [목적어: ex:뉴진스] ──(ex:국적)──> ["대한민국"]
```
...
```mermaid
flowchart TD
    User["1. 사용자 질문: 민서가 들은 곡과 가수 국적은?"] --> Engine["2. GraphRAG 엔진"]
    Engine -->|SPARQL 사실 질의| KG[("3. 지식 그래프\n[민서 ➔ Ditto ➔ 뉴진스 (대한민국)]")]
    KG -->|검증 팩트 반환| Engine
    Engine -->|팩트 주입 프롬프트 합성| LLM["4. LLM 에이전트"]
    LLM -->|환각 0% 근거 기반 답변| Ans["5. '민서 님은 뉴진스의 Ditto를 들었으며 뉴진스는 한국 국적입니다'"]
```

```text
[1. 사용자 질문] ──> [2. GraphRAG 엔진] ──(SPARQL 사실 쿼리)──> [3. 지식 그래프 (검증 팩트 반환)]
                            │
                            └──(팩트 주입 프롬프트 전달)──> [4. LLM] ──> [5. 환각 0% 정확한 답변]
```
