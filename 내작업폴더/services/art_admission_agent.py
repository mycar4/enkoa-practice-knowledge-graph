...
get_competition_rate_ranking으로 경쟁률이 낮은 학교를 보여줄 때, "안정적으로 지원할
수 있는 좋은 선택"처럼 합격 가능성을 단정하는 표현을 쓰지 마십시오 - 경쟁률은 그
해 지원자 수만 반영할 뿐 학생 개인의 성적/실기 수준과 무관하고, 학년도·모집인원에
따라 다음 해 경쟁률이 크게 달라질 수 있습니다(실측 발견: 경쟁률 2.0:1인 학교를
"안정적으로 지원할 수 있는 좋은 선택"이라고 근거 없이 단정한 사고가 있었습니다).
경쟁률 숫자만 사실대로 전달하고, "경쟁률만으로 합격 가능성을 판단할 수 없으니
본인 성적·실기 준비 상태를 함께 고려하라"고 덧붙이십시오.

recommend_by_grades 결과를 답할 때 지켜야 할 3가지(실측 QC에서 전부 지적된 항목):
(1) 사용자가 준비 중인 실기 종목을 말했으면(예: "소묘 준비 중"), **실기가 아예 없는
    전형(exam_type_name이 "비실기"/"학생부 100%"/"실기 없음"인 것)은 추천 목록에서
    빼십시오** - 실측 사고: 소묘 준비생에게 "목원대 교과전형(비실기, 학생부 100%)"을
    추천해 실기 준비가 전혀 쓸모없는 전형을 권했습니다. 굳이 언급하려면 "실기 없이
    학생부만으로 가는 대안"이라고 성격을 분명히 구분해 따로 적으십시오.
(2) 단순히 학교·전형을 나열하지 말고, **왜 이 학생 성적에서 가능한지**를 도구가 준
    값으로 설명하십시오(실기 비중/학생부 비중, school_record_percentage,
    reason_summary 등). 성적을 말한 사용자는 "가능한 목록"이 아니라 "왜 가능한지"를
    알고 싶어 합니다.
(3) 실기 종목을 아직 안 말한 사용자에게는 결과 끝에 반드시
    "준비 중인 실기 종목(예: 소묘/기초디자인)을 알려주시면 더 좁혀드릴게요"라고
    덧붙이십시오 - 종목마다 지원 가능 전형이 완전히 달라지기 때문입니다.

사용자가 "계속 같은 얘기만 한다", "다른 것도 알려달라"처럼 답변이 반복적이라고
불만을 표하면, 사과만 하고 되묻지 마십시오. 이 서비스가 실제로 해줄 수 있는 일을
구체적으로 제시하십시오 - 예: 성적 기반 지원 추천, 실기 종목별 지원 가능 학교 검색,
두 대학 실기고사일 충돌 확인, 대학별 전형 일정/모집인원 조회, 기출문제 확인,
서류(미술활동보고서/자기소개서) 첨삭 화면 안내. 그중 사용자가 아직 안 물어본 것을
2~3개 골라 "이런 것도 확인해드릴 수 있습니다"라고 먼저 제안하십시오.
...
def get_competition_rate_ranking(topic_keywords: List[str] = [], top_n: int = 10, order: str = "desc") -> str:
    ...
    순으로 정렬한다(경쟁률 숫자만 보여줄 뿐, "안정적"/"합격 가능성이 높다"는 의미가
    아니다 - 경쟁률은 학년도·모집인원·지원자 성적 분포에 따라 실제 합격선과 무관할
    수 있다). competition_rate는 배수(예: 9.03은 "9.03:1", 9.03%가
    아님) - 답변에 %를 붙이지 말 것.
...
def run_agent(query: str, history: Optional[List[Dict[str, str]]] = None) -> Dict[str, Any]:
    ...
    # 2026-09-23 QC 후속: 실기 종목을 말하지 않은 성적 질의에는 "종목을 알려주면 더
    # 좁혀준다"는 안내를 반드시 붙여야 한다(종목마다 지원 가능 전형이 완전히 달라짐).
    # 시스템 프롬프트에 두 번 명시했는데도 gpt-4o-mini가 계속 빠뜨리는 것을 실측으로
    # 확인해, 프롬프트 의존을 포기하고 코드로 결정론적으로 덧붙인다(오늘 동명 전형
    # 구분·대학 수 집계에서도 같은 판단을 했다 - 반드시 지켜야 하는 것은 코드로 건다).
    _used_grades_tool_without_topic = any(
        t.get("tool") == "recommend_by_grades"
        and not (t.get("args") or {}).get("topic_keywords")
        and not (t.get("args") or {}).get("material_query")
        for t in tool_trace
    )
    if _used_grades_tool_without_topic and "종목" not in final_answer[-200:]:
        final_answer += (
            "\n\n준비 중인 실기 종목(예: 소묘·기초디자인·수채화)을 알려주시면 "
            "그 종목으로 지원할 수 있는 전형만 골라 더 좁혀드릴 수 있습니다."
        )
