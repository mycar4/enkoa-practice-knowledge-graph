...
class ArtAdmissionService:
        ...
        with self.driver.session(default_access_mode=READ_ACCESS) as s:
            rows = s.run("""
                ...
                RETURN u.name AS university, u.campus AS campus, d.name AS department, t.name AS track_name,
                       ...
                       t.stage_ratio_label AS stage_ratio_label,
                       t.stage_practical_ratio_pct AS stage_practical_ratio_pct,
                       t.stage_school_record_ratio_pct AS stage_school_record_ratio_pct,
                       t.competition_rate AS competition_rate,
                       t.competition_applicant_count AS competition_applicant_count,
                       t.competition_rate_announced_at AS competition_rate_announced_at,
                       t.competition_rate_source_url AS competition_rate_source_url,
    ...
    def search_document_excerpts(self, query: str, top_k: int = 5, candidate_pool: int = 15,
        ...
        (hybrid_search_auto_merge)을 우선 시도하고, 결과가 비면(아직 청킹 안 된
        학교 등) 기존 hybrid_search()(부모 청크 단독)로 자동 폴백한다. 호출부는
        롤아웃 진행 상황을 몰라도 이 함수 하나만 쓰면 된다.

        2026-09-22: 예전엔(child chunk가 홍익대 1개교에만 있던 2026-09-21 시점)
        universities가 없는(=학교명 미특정) 질문에서 auto_merge를 아예 스킵하고
        곧장 구식 hybrid_search로 보냈다 - 전체 55개교 대상 검색에서 자식청크가
        홍익대에만 있으면 나머지 54개교가 결과에서 통째로 빠지는 회귀가 있었기
        때문. 지금은 day46 롤아웃으로 55개교 전부 child chunk가 있어서 그 회귀
        조건 자체가 사라졌는데도 이 가드가 남아 있어, 학교명 없는 질문 전부가
        계속 저정밀(1500자 단위) 검색만 타고 있었다(실측 발견: "미술활동보고서
        표절 기준이 뭐야?"에서 구식 경로는 표절 판정 수치표를 5개 결과 중 하나도
        못 찾았고, auto_merge는 1위로 정확히 찾아냄) - 이제 학교명 특정 여부와
        무관하게 항상 auto_merge를 먼저 시도한다."""
    ...
    # 낭비했다. 키워드를 분량/글자수/기재금지 같은 실제 작성 제약어로 좁힌다.
    _DOC_RULE_KEYWORDS: Dict[str, List[str]] = {
        "자기소개서": ["블라인드", "분량", "글자", "표절", "대필", "이내", "기재하지", "노출"],
        "미술활동보고서": ["블라인드", "분량", "글자", "표절", "대필", "이내", "기재하지", "노출"],
        "포트폴리오 설명글": ["포트폴리오", "블라인드", "분량", "글자", "이내", "기재하지", "노출"],
        "기타 서류": ["블라인드", "분량", "글자", "표절", "대필", "이내"],
    }

    # 위 키워드로도 걸러지는 절차/FAQ성 노이즈(평가자 회원가입, 로그인 방법 등) -
    # 지원자가 글을 "어떻게 써야 하는가"와 무관하므로 실제 작성규정 청크가 있으면
    # 이쪽을 우선 제외한다(작성규정 청크가 하나도 없을 때만 폴백으로 포함).
    _DOC_RULE_NOISE_PATTERNS: List[str] = [
        "회원가입", "확인서를 제출", "재직경력", "평가요청", "로그인", "아이디",
        "비밀번호", "Q. ", "Q.\n", "홈페이지에 접속하여",
    ]
    ...
    def get_document_rule_excerpts(self, university: str, doc_type: str, top_k: int = 6) -> List[Dict[str, Any]]:
        ...
        real_rows = real_rows or rows
        # 2026-09-21: 평가자 회원가입/로그인/확인서 제출 같은 절차 안내(FAQ)는
        # "지원자가 이 글을 어떻게 써야 하는가"와 무관하다 - 실제 작성규정 청크가
        # 하나라도 있으면 이런 노이즈는 빼서 LLM에 넘기는 토큰을 줄이고, 상위
        # top_k가 무관한 절차 안내로 채워지지 않게 한다. 다 노이즈뿐이면(즉 이
        # 학교 문서에 절차 안내밖에 없으면) 그대로 폴백해서 빈 결과를 만들지 않는다.
        content_rows = [
    ...
    def recommend_universities(self, grades: List[Dict[str, Any]],
        ...
        for t in tracks:
            entry = {
                ...
                "stage_ratio_label": t.get("stage_ratio_label"),
                "stage_practical_ratio_pct": t.get("stage_practical_ratio_pct"),
                "stage_school_record_ratio_pct": t.get("stage_school_record_ratio_pct"),
            }
            doc_category = self._document_track_category(t.get("exam_type_name"))
