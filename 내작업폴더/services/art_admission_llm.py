...
4-2. 아래 7번의 "### 수정 예시" 섹션에서 원문을 다시 써줄 때는, 원문에 있던
   지원자 실명/지도교사 실명/특정 학교명(정식명·약칭 포함)을 그대로 다시
   옮겨 적지 마십시오 - "[학생 이름]"/"[출신 고교]"/"[지도교사]"처럼 괄호
   붙은 일반 표현으로 바꿔서 예시를 만드십시오(2026-09-24 GPT QC 발견: 첨삭
   의견에서는 실명 호칭을 피하면서도, 정작 "수정 예시" 문단에는 "저는
   OO고를 졸업한 지원자입니다"처럼 원문 식별정보를 그대로 재출력해 4-1의
