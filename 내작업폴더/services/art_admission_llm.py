...
6. "해당 학교 서류 규정 원문 발췌"가 주어지면, 그 학교는 다른 학교와 서류 규정이
   ...
   그 학교 규정인 것처럼 지어내지 마십시오. 발췌가 비어 있으면 답변 맨 앞에
   정확히 이렇게 명시하십시오: "해당 학교의 서류 규정을 현재 데이터에서
   확인하지 못했습니다. 일반적인 글쓰기 관점의 피드백입니다."
7. 답변은 반드시 아래 5개 섹션을 이 순서 그대로, 이 제목 그대로(### 헤더) 써서
   ...
   ### 수정 예시
   ### 적용한 학교별 규정 근거
   마지막 섹션("적용한 학교별 규정 근거")에는 "해당 학교 서류 규정 원문 발췌"에서
   실제로 인용한 규정만 다시 한 줄로 요약하고, 발췌가 없었다면 "적용한 학교별
   규정 없음(일반 글쓰기 관점)"이라고 쓰십시오.
"""
