...
* **사모사채 및 연계 지분 이동 경로 추적 부재**:  
  사모 전환사채(CB), 신주인수권부사채(BW), 제3자배정 유상증자, 타법인 주식 양수도로 이어지는 복합 자본 변동 경로는 단일 공시 건별로는 정상 공시로 보이지만, **시간 축과 지분망을 그래프로 꿰뚫어 보아야만 다단계 자본 이동 경로**가 드러납니다.
...
* **핵심 업무**: 1차 파일럿 95개 상장사 전수 공시 인덱스 및 정형 지분/출자 통합
* **다루는 OpenDART API**:
...
### [v0.3] 기업 주요 자본 이벤트 및 자금 흐름 추적 (완료 🟢)
* **핵심 업무**: 파일럿 95개 상장사 대상 5대 주요사항 공시 시계열 이벤트 노드 적재
* **다루는 OpenDART API (DS005 주요사항보고서 5대 핵심)**:
  1. `cvbdIsDecsn`: 전환사채(CB) 발행결정 (118건)
  2. `bdwtIsDecsn`: 신주인수권부사채(BW) 발행결정 (15건)
  3. `piicDecsn`: 유상증자결정 (110건)
  4. `otcprStkInvscrInhDecsn`: 타법인 주식 및 출자증권 양수결정 (23건)
  5. `cmpMgDecsn`: 회사합병결정 (47건)
* **온톨로지 실측 적재 성과**:
  * **`:DART_CapitalEvent` 노드 총 313건** 및 공시 원문 `[:EVIDENCED_BY]` **313건 100% 전수 연결**
  * 3원 일자 분리 적재 (`decided_on`: 201건, `received_on`: 313건 필수 100%, `effective_on`: 174건)
  * 파생 프로젝션 엣지: **정확한 1:1 매칭 기준** 엄격 적용 (`MERGED_WITH` 4건)
  * *(참고: 인수자 세부 연결 `[:SUBSCRIBED]`는 비정형 텍스트 정밀 파서가 요구되므로 v0.3-Phase 2 백로그로 이관)*
